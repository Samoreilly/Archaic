...
static int send_request(ipc_client* client, uint32_t type, const void* payload, size_t len) {
    ...
    if (write_exact(client->fd, &hdr, sizeof(hdr)) < 0)
        return -1;
    if (payload && len > 0) {
        if (write_exact(client->fd, payload, len) < 0)
            return -1;
    }
    return 0;
}
...
static int recv_response(ipc_client* client, ipc_header* hdr, void* payload, size_t max_len) {
    ...
    if (hdr->payload_len > 0) {
        if (read_exact(client->fd, payload, hdr->payload_len) < 0)
            return -1;
    }
    return 0;
}
...
int ipc_client_scan(ipc_client* client, const char* path) {
    ...
    if (send_request(client, IPC_MSG_SCAN, &req, sizeof(req)) < 0)
        return -1;
...
int ipc_client_save(ipc_client* client, const char* path) {
    ...
    if (send_request(client, IPC_MSG_SAVE, &req, sizeof(req)) < 0)
        return -1;
...
int ipc_client_query(ipc_client* client, const char* cwd, const char* input,
    ...
    if (send_request(client, IPC_MSG_QUERY, &req, sizeof(req)) < 0)
        return -1;
...
int ipc_client_shutdown(ipc_client* client) {
    if (send_request(client, IPC_MSG_SHUTDOWN, NULL, 0) < 0)
        return -1;
...
int ipc_client_ping(ipc_client* client, uint64_t* out_uptime_ms) {
    ipc_header hdr;
    ipc_pong_resp resp;
    if (send_request(client, IPC_MSG_PING, NULL, 0) < 0)
        return -1;
    if (recv_response(client, &hdr, &resp, sizeof(resp)) < 0)
        return -1;
    if (hdr.msg_type != IPC_MSG_PONG)
        return -1;
    if (out_uptime_ms)
        *out_uptime_ms = resp.uptime_ms;
    return 0;
}

int ipc_client_metrics(ipc_client* client, ipc_metrics_resp* out) {
    ipc_header hdr;
    if (send_request(client, IPC_MSG_METRICS, NULL, 0) < 0)
        return -1;
    if (recv_response(client, &hdr, out, sizeof(*out)) < 0)
        return -1;
...
int ipc_client_scan_status(ipc_client* client, ipc_scan_status_resp* out) {
    ipc_header hdr;
    if (send_request(client, IPC_MSG_SCAN_STATUS, NULL, 0) < 0)
        return -1;
    if (recv_response(client, &hdr, out, sizeof(*out)) < 0)
        return -1;
...
int ipc_client_fuzzy(ipc_client* client, const char* query, uint32_t limit,
    ...
    if (send_request(client, IPC_MSG_FUZZY_COMPLETE, &req, sizeof(req)) < 0)
        return -1;
