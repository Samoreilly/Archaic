...
void config_init_defaults(archaic_config* cfg) {
    ...
    strncpy(cfg->daemon.socket_path, "/tmp/archaic-daemon.sock", sizeof(cfg->daemon.socket_path) - 1);
...
static int parse_int(const char* s, int* out) {
    ...
    if (errno || end == s || *trim(end) != '\0') return -1;
    *out = (int)val;
    return 0;
}
...
#define FOFFSET(section, field) ((void*)(offsetof(archaic_config, section) + offsetof(typeof(((archaic_config*)0)->section), field)))

static int set_field(archaic_config* cfg, const field_map* map, int map_len, const char* key, char* value) {
    for (int i = 0; i < map_len; i++) {
        if (strcmp(map[i].key, key) != 0) continue;
...
int config_load_default(archaic_config* cfg) {
    ...
    if (env && *env) {
        if (config_load(cfg, env) == 0) return 0;
    }
    ...
    if (config_load(cfg, "/etc/archaic/config.toml") == 0) return 0;
